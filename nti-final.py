    ...
    def set_arrow(self, v, depth, hsv):
        ...
        while True:
            ...
            if self.time() - offset_timer >= 0.7:
                ...
                return 0
    def set_obj(self, v, depth, hsv, x = 0, y = 0, corners=4):
        mur = self.mur
        res = self.res
        offset_timer = self.time()
        from time import sleep
        while True:
            pos = v.detect_object(mur.get_image_bottom(), orange)#v.obj(mur.get_image_bottom(), hsv, corners=corners)
            if pos is None:
                return -1
...
class vision():
    ...
    def detect_object(self, image, hsv):
        from math import pi
...
orange = np.array([np.array([5, 83, 107]), np.array([33, 246, 255])]) #np.array([np.array([18, 139, 0]), np.array([21, 255, 255])])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        while f.set_stand(v,-2.8, blue) == -1:
            continue
        color_cube = orange
        mur.open_grabber()
        f.set(depth = -3.2)
        ...
        f.set(depth = -2.6)
        #time.sleep(1)
        while f.set_arrow(v,a_height,green) == -1:
            continue
        ang = mur.get_yaw()
        timer = time.time()
        while time.time() - timer <= 1:
            f.keep_angle(ang,60)
            f.keep_depth(-2.6)
        while f.set_stand(v, -2.6, green) == -1:
            f.keep_angle(ang,60)
